...
SELECT 
	CEIL( (SYSDATE - TO_DATE('1995-01-16', 'YYYY-MM-DD')) / 365 ) AS 나이
FROM DUAL;
...
-- EMPLOYEE 테이블에서
-- 입사일이 '2000-01-01' 부터 '2000-12-31'인 사이인 사원의
-- 이름, 입사일 조회
SELECT EMP_NAME, HIRE_DATE 
FROM   EMPLOYEE
...
WHERE PHONE LIKE '010________'; -- ___ 버전
...
SELECT EMP_NAME, PHONE FROM EMPLOYEE
ORDER BY PHONE DESC NULLS FIRST; -- 정령 기준 -> NULL 위치 순서대로 해석
